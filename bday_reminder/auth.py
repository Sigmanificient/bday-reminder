...
PASSWORD_PATTERN = r"^.*(?=.{8,})(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[-_ @#$%^&+=]).*$"
    ...
    if request.method == "POST":
        ...
        if username and password:
            login = User.query.filter_by(
                pseudo=username, password=blake2b(password)
            ).first()
...
def register_page() -> Redirect_or_Webpage:
    ...
    if request.method == "POST":
        ...
        if (
            ...
            new_user = User(
                pseudo=username, password=blake2b(password), birthday=birthday
            )
...
def dashboard_page() -> Redirect_or_Webpage:
    ...
    if not today_birthdays:
        return render_template("auth/dashboard.jinja2", birthdays=birthdays)

    if len(today_birthdays) == 1:
        displayed = today_birthdays[-1]

    else:
        displayed = ", ".join(today_birthdays[:-1]) + f" and {today_birthdays[-1]}" * (
            len(today_birthdays) > 2
        )

    return render_template(
        "auth/dashboard.jinja2", birthdays=birthdays, today_birthdays=displayed
    )
...
def delete_user(index: str) -> Json:
    ...
    if not user or not user.get("name") or not index.isdigit():
        return {}

    db.session.delete(
        Birthday.query.filter_by(user_id=user.get("id"), id=int(index)).first()
    )
